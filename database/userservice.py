...
# функц для проверки на наличие пользователя
def check_user_db(name, phone_number, email):
    ...
    # ф-ия для рег пользователя
def register_user_db(name, phone_number, email, password, birthday, user_city):
    with next(get_db()) as db:
        checker = check_user_db(name, phone_number, email)
        if checker == True:
            new_user = User(name=name, phone_number=phone_number, email=email,
                            password=password, birthday=birthday, user_city=user_city)
            db.add(new_user)
            db.commit()
            return new_user.id
        return checker

    # ф-ия для авторизации
